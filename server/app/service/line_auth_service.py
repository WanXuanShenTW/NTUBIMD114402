...
PENDING_TTL = int(os.getenv("LINE_BIND_TTL", 300))           # 秒，預設 5 分鐘
MAX_ATTEMPTS = int(os.getenv("LINE_BIND_MAX_ATTEMPTS", 5))   # 密碼重試上限

# 可選 bcrypt（若沒裝會自動用純文字比較）
try:
...
async def start_phone_step(line_user_id: str, text: str) -> Tuple[bool, str]:
    ...
    user = await get_user_by_phone(text)  # 回傳 {user_id, name, role_id}
    if not user:
...
def _check_password(plain: str, stored: Optional[str]) -> bool:
    """
    比對密碼：
      - 若 stored 以 $2 開頭且系統裝了 bcrypt → 用 bcrypt 驗證
      - 否則退回純文字比較（請依你實際的雜湊機制自行調整）
    """
    if not stored:
        return False
...
async def confirm_password_step(line_user_id: str, password: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    """
    第二步：輸入密碼驗證
    通過 → upsert 綁定、清除暫存，回 user
    失敗 → 增加重試次數；超過上限或過期要求重來
    """
    st = PENDING.get(line_user_id)
    ...
    PENDING.pop(line_user_id, None)
    user = {"user_id": user_id, "name": user_name, "role_id": 1}
