# -*- coding: utf-8 -*-
"""
binary_cnn_lstm_loader.py (對齊 Trainer v4 與 Multi Loader)

功能：
- ✅ 自動讀取同名影片檔以鎖定解析度 (解決座標抖動)
- ✅ 對齊 Motion Clip, GroupNorm, Channel 數
- ✅ 移除 Kalman，改用插值補點
- ✅ 修正：現在與 Multi Loader 一樣具備自動解析度偵測功能

使用方式：
1. 修改 MODEL_PATH, POSE_JSON, OBJECT_JSON
2. python binary_cnn_lstm_loader.py
"""
...
# 影片根目錄 (用於讀取解析度)
# [修正] 請確認這裡的路徑指向你影片的根目錄 (例如 medias/train_video/binary)
VIDEO_ROOT  = "medias/raw_n_edited/fall_all" 
...
def try_get_video_resolution(json_path):
    ...
    found_path = None
    # 策略 1: 直接在 VIDEO_ROOT 下找 (需要 VIDEO_ROOT 設定正確)
...
def run_on_json(pose_json: str, object_json: Optional[str]=None):
    ...
    # fixed_w, fixed_h = 320, 240  # 如果需要強制鎖定 (Debug 用)，可以解開這行
