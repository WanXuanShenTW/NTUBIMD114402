...
OBJECT_CLASSES     = [s.strip() for s in os.getenv("SC_OBJECT_CLASSES", "bed,chair,bench").split(",") if s.strip()]
...
def _interp_frame(prev_rec: Dict[str, Any], curr_rec: Dict[str, Any], alpha: float = 0.5) -> Dict[str, Any]:
    ...
    for i in range(17):
        ...
        kps.append({"x": _lin(px, cx, alpha), "y": _lin(py, cy, alpha), "conf": _lin(pc, cc, alpha)})
    ...
    ts_mid = None
    try:
        p_ts = prev_rec.get("ts_ms"); c_ts = curr_rec.get("ts_ms")
        ...
        if not INTERP_DOUBLE:
            ...
            if prev_real is not None and (prev_real.get("frame_id") is not None) and (fid_int is not None):
                ...
                if prev_id is not None:
                    ...
                    synth_seq = 2 * prev_id + 1     # 奇數序：夾在 2*prev_id 與 2*fid_int 之間
    ...
    async def _run_two_stage(self, user_id: str, clip_bin: List[Dict[str, Any]]):
        async with self._locks[user_id]:
            ...
            if bin_pred != "fall":
                ...
                if tail_seq_cur is None or tail_seq_cur != self._last_multi_tail_seq[user_id]:
