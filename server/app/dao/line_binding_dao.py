# app/dao/line_binding_dao.py
from typing import Optional, Dict, Any
from app.db import Database

async def upsert_binding(user_id: int, line_user_id: str) -> None:
    """
    一對一綁定（不改 schema）：
...
async def get_user_by_line_user_id(line_user_id: str) -> Optional[Dict[str, Any]]:
    """
...
async def get_line_user_id_by_user(user_id: int) -> Optional[str]:
    """
...
async def unbind_by_line_user_id(line_user_id: str) -> int:
    async with Database.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
