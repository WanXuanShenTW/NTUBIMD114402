...
class Config:
    ...
    out_dir   = "outputs/models/test/multi"
    ...
    class_whitelist = None  # None 或 list[str]；設定時 blacklist 必須為 None
    class_blacklist = ["lay","seated"]  # None 或 list[str]；設定時 whitelist 必須為 None
...
class PoseObjectDataset(Dataset):
    def __init__(self, cfg: Config):
        ...
        # 掃描 pose_root 和 obj_root 的類別資料夾
        ...
        # 注意：只有在非 None 時才視為「有設定」
        if wl is not None:
            ws = {str(w).strip() for w in wl if w is not None and str(w).strip()}
            self.class_names = [c for c in self.class_names if c in ws]

        elif bl is not None:
            bs = {str(b).strip() for b in bl if b is not None and str(b).strip()}
            self.class_names = [c for c in self.class_names if c not in bs]

        if not self.class_names:
            raise RuntimeError("篩選後沒有可用的類別資料夾（請檢查 whitelist/blacklist 設定與資料夾名稱）")
